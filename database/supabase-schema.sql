...
create policy "friends_manage_own" on public.community_friends for all using (auth.uid()=owner_id) with check (auth.uid()=owner_id);
create policy "encouragements_insert_own" on public.encouragements for insert with check (auth.uid()=sender_id);
create policy "encouragements_read_related" on public.encouragements for select using (auth.uid()=sender_id or auth.uid()=recipient_id);
...
returns table (user_id uuid,full_name text,gender text,level integer,points integer,is_current_user boolean)
language sql stable security definer set search_path=''
as $$
  select p.id,coalesce(nullif(p.full_name,''),'สมาชิกชุมชน'),coalesce(p.gender,'หญิง'),
    greatest(1,least(100,1+floor(greatest(0,coalesce((s.data->>'gardenBonus')::integer,0)+coalesce((select sum(coalesce((x->>'points')::integer,10)) from jsonb_array_elements(coalesce(s.data->'prayerHistory','[]'::jsonb)) x),0))/100.0)::integer)),
    greatest(0,coalesce((s.data->>'gardenBonus')::integer,0)+coalesce((select sum(coalesce((x->>'points')::integer,10)) from jsonb_array_elements(coalesce(s.data->'prayerHistory','[]'::jsonb)) x),0))::integer,
    p.id=auth.uid()
  from public.profiles p join public.user_state s on s.user_id=p.id order by 5 desc limit 100;
$$;
